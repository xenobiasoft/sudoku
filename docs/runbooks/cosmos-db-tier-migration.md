...
| ------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Date**     | 2026-07-09                                                                                                                                                                                                                                                                                                                                                                                           |
| **Status**   | **Cutover complete (2026-07-10 00:08 UTC); Phase 6 now unblocked.** Phases 0-4 done; Phase 7 code cleanup shipped (#357). Production serves exclusively from `cosmos-sudoku-prod2` (serverless) — App Insights over the 7 days to 2026-07-27 shows 1,564 Cosmos dependency calls on the new account, **zero on `cosmos-sudoku-prod`**, 0.26% failure rate. The 1-2 week clean-operation gate expired 2026-07-24. Remaining: (1) verify and delete the stale App Config keys in `appcs-xenobiasoft-prod` — the ✅ on that Phase 7 bullet contradicts its own text, so confirm in Azure rather than trusting either; (2) Phase 6, delete the old account — **irreversible**, destroys the rollback path and the old account's backups. |
...
## Phase 1 — Provision the new account via Bicep ✅ complete
...
## Phase 2 — Grant access on the new account ✅ complete
...
1. Load a game that existed _before_ the migration. It must open normally. If it 404s, roll back (Phase 5).
...
## Phase 4 — Post-cutover validation ✅ complete 2026-07-18

- **Confirm you are actually on the new account.** The failure mode this runbook exists to prevent is a cutover that silently didn't happen — and a cutover that didn't happen produces _no errors at all_. `CosmosDbService.InitializeCosmosDbAsync` logs the endpoint it connected to (`CosmosDbService.cs:180`), so check it directly:
  ```
  az webapp log tail --name XenobiasoftSudokuApi-prod -g rg-xenobiasoft-sudoku-prod-westus2 \
